---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the patreon-archiver project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the patreon-archiver project.
...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'patreon_archiver': {}})
  ```
...
- `` :py:mod:`patreon_archiver.string` `` for modules
- `` :py:func:`patreon_archiver.string.slugify` `` for functions
- `` :py:class:`patreon_archiver.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~patreon_archiver.string.slugify` `` renders as `slugify`
