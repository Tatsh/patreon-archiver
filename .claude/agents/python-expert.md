...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  ```python
  setup_logging(debug=debug, loggers={'patreon_archiver': {}, 'urllib3': {}, 'soupsieve': {}})
  ```
...
- Use `~` to shorten the displayed name: `` :py:func:`~patreon_archiver.string.slugify` `` renders as
  `slugify`
